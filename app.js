...
class Chat {
    ...
    onApiLoad(error, data)
    {
        ...
        this.ws = new WebSocket("ws://wolfdev.cbenni.com:8016/");
        ...
        else
        {
            ...
            for (var emoteset in data.emoticon_sets)
            {
                if (data.emoticon_sets.hasOwnProperty(emoteset))
                {
                    ...
                    {
                        var re = '(?!<a [^>]*?>)';
                        if (data.emoticon_sets[emoteset][i].code.match(/^\w/))
                        {
                            re += '\\b';
                        }
                        re += '(' + data.emoticon_sets[emoteset][i].code + ')';
                        if (data.emoticon_sets[emoteset][i].code.match(/\w$/))
                        {
                            re += '\\b';
                        }
    ...
    registerEventHandlers()
    {
        ...
        {
            $('#messages span.user[data-name=' + $(this).attr('data-name') + ']').parent().css("background-color", "#FF0");
        });
...
$(document).ready(function ()
{
    ...
    Twitch.init({
        clientId: "gtby98ee11ozz5rsytulvltqawkcp0o"
